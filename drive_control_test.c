...
#pragma config(Sensor, dgtl1,  chassisSonar,   sensorSONAR_mm)
...
task main() {
  writeDebugStreamLine("im working");

  driveControlStart();
  driveControlMoveIntoBuckyScoringPosition();

  while ( !driveControlErrorStable ) {
    wait1Msec(25);
  }
